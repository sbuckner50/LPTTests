            ...
            if solver == QPSolvers.PDIPM_BATCHED:
                ...
                for i in range(nBatch):
                    ...
                    if model is None:
                        ...
                        vals[i], zhati, nui, lami, si = forward_gurobi_prebuilt(Q[i].detach().numpy(), p[i].detach().numpy(), 
                                     model, x, inequality_constraints, equality_constraints, Gi, hi, quadobj)
                    # if zhati[0] is None:
...
class SpQPFunction(Function):
    def __init__(self, Qi, Qsz, Gi, Gsz, Ai, Asz, eps=1e-12, verbose=0, notImprovedLim=3, maxIter=20):
